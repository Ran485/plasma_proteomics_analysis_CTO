...
def process_and_plot_data(inpath, outpath, time_name, colors):
    ...
    data_count["cohort"] = data.columns.get_level_values(0).tolist()
    ...
    boxplot_data.columns = pd.MultiIndex.from_tuples(
        boxplot_data.columns, names=["Exp", "Rep"]
    )
    # boxplot_data = boxplot_data.stack(level="Rep").reset_index()
    boxplot_data = boxplot_data.unstack().reset_index()
    boxplot_data.rename(columns={0: "iFOT"}, inplace=True)
    boxplot_data["iFOT"] = 2 ** boxplot_data["iFOT"]
...
if __name__ == "__main__":
    ...
    # The input file path for the protein data
