...
def intensity_plot(
    data, project, color, highlight_proteins, axis_max, output, time_name
):
    """
    ...
    mean_intensity = np.log10(
        numeric_data.mean(axis=1).sort_values(ascending=False)
    ).to_frame()
    mean_intensity.rename(
        columns={0: "Quantitative protein intensity[Log10]"}, inplace=True
    )
    ...
    for protein in highlight_proteins:
        if protein in mean_intensity.index:
            ...
            ax.annotate(
                protein,
                xy=(
                    position["Rank of quantified proteins"],
                    position["Quantitative protein intensity[Log10]"],
                ),
                xytext=(5, 0),
    ...
    for fmt in [".png", ".pdf", ".eps"]:
        plt.savefig(
            os.path.join(output, f"proteins_{project}_{time_name}{fmt}"),
            transparent=True,
        )
    plt.show()
...
def plot_protein_intensity(inpath, outpath, config_path=None):
    ...
    for cohort, color in [
        ...
        intensity_plot(
            data.get(cohort, data),
            cohort,
            color,
            protein_lists["represent_proteins"],
            8000,
            outpath,
            time_name,
        )
...
if __name__ == "__main__":
    inpath = r'/Volumes/Samsung_T5/xinjiang_CTO/merge-data/Protein_matrix_rawdata_sorted_dynamic.csv'
    # The results of the analysis are saved in the following path
    outpath = r'/Volumes/Samsung_T5/xinjiang_CTO/figs-1/'
